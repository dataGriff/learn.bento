...
input:
  file:
    paths:
      - ./examples/04-bloblang-transform/data/events.jsonl
    codec: lines
...
output:
  broker:
    ...
    outputs:
      ...
      - file:
          path:  ./examples/06-fan-out-broker/out/raw.jsonl
          codec: lines
      ...
      - processors:
        ...
        file:
          path:  ./examples/06-fan-out-broker/out/audit.log
          codec: lines
      ...
      - processors:
          - mapping: |
              ...
              root.tier  = this.tier
        file:
          path:  ./examples/06-fan-out-broker/out/summary.jsonl
          codec: lines
```
...
```bash
cat examples/06-fan-out-broker/out/raw.jsonl
cat examples/06-fan-out-broker/out/audit.log
cat examples/06-fan-out-broker/out/summary.jsonl
```
